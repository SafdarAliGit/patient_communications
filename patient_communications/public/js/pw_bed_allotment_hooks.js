// Injected by patient_communications — adds "Send Patient Comms Link" to PW Bed Allotment
frappe.ui.form.on("PW Bed Allotment", {
	refresh(frm) {
		if (!frm.doc.patient || frm.doc.status !== "Active") return;

		frm.add_custom_button(__("Send Patient Comms Link"), function () {
			frappe.call({
				method: "patient_communications.api.get_communication_link",
				args: { customer: frm.doc.patient },
				freeze: true,
						...
						fields: [
							{
								...
								<div style="margin-bottom:12px;">
									<label style="font-size:11px;font-weight:600;color:#6b7280;text-transform:uppercase;letter-spacing:.04em;">Portal Link</label>
									<div style="display:flex;align-items:center;gap:8px;margin-top:4px;">
										<input id="pc-link-input" type="text" value="${frappe.utils.escape_html(d.portal_url)}"
											readonly style="flex:1;padding:6px 10px;border:1px solid #d1d5db;border-radius:6px;font-size:13px;background:#f9fafb;">
